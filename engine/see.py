...
def see(board: chess.Board, move: chess.Move, threshold: int = 0) -> int:
    ...
    # Improved: Handle en passant correctly
    if board.is_en_passant(move):
        ep_sq = chess.square(chess.square_file(tgt), chess.square_rank(move.from_square))
        gain[0] = _VALUES[chess.PAWN]
        occ ^= chess.BB_SQUARES[ep_sq]  # Remove captured pawn
